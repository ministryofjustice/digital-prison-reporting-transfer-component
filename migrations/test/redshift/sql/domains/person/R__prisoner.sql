...
- cell_location
- ethnicity, religion, nationality and gender

TODO
1: To use the internal locations service when available in production - 
-- LEFT JOIN prisons.locationsinsideprison_location l ON l.prison_id = ob.agy_loc_id
...
WITH
    ofrel AS (
        ...
        FROM
            prisons.nomis_offender_profile_details
            LEFT JOIN prisons.nomis_profile_codes ON prisons.nomis_profile_codes.profile_code = prisons.nomis_offender_profile_details.profile_code
        where
            prisons.nomis_offender_profile_details.profile_type = 'RELF'
    ),
    ethnicity_domain AS (
        SELECT
            code,
    ...
    nationality_info AS (
        ...
        where
            prisons.nomis_offender_profile_details.profile_type = 'NAT'
    ),
    gender_domain AS (
        SELECT
            code,
...
SELECT
    o.offender_id_display as prisoner_number,
    o.first_name,
    o.last_name,
    ...
    ob.active_flag AS active
FROM
    prisons.nomis_offender_bookings ob
