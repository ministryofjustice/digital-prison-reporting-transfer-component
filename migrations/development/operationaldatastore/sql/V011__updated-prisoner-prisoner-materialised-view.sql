...
CREATE UNIQUE INDEX prisoner_prisoner_offender_id_key ON prisoner.prisoner(offender_id);
SELECT cron.schedule ('refresh prisoner.prisoner','15 2 * * *','REFRESH MATERIALIZED VIEW CONCURRENTLY prisoner.prisoner');

GRANT USAGE ON SCHEMA prisoner TO fdw_access_domain_read_only;
GRANT SELECT ON ALL TABLES IN SCHEMA prisoner TO fdw_access_domain_read_only;
ALTER DEFAULT PRIVILEGES IN SCHEMA prisoner GRANT SELECT ON TABLES TO fdw_access_domain_read_only;
